...
function hashString(s) {
  const h = sha2562(new TextEncoder().encode(s));
...
function createCommitment(intent, revealDelayMs, privateKey, publicKey) {
  ...
  const hash = sha2563(new TextEncoder().encode(intentCanonical));
...
function computeAnchorHash(marker) {
  ...
  return toHex(sha2564(new TextEncoder().encode(canonical)));
}
...
function hashField(value) {
  ...
  const hash = sha2565(new TextEncoder().encode(canonical));
...
function hashPair(a, b) {
  ...
  return toHex3(sha2566(new TextEncoder().encode(combined)));
}
...
function digestKey(publicKey) {
  return toHex4(sha2567(publicKey));
}
function createInception(opts) {
  const identifier = `did:keri:${toHex4(sha2567(opts.keys[0])).slice(0, 32)}`;
  const timestamp = (/* @__PURE__ */ new Date()).toISOString();
  const event = {
    type: "inception",
    identifier,
...
function createRotation(currentState, newKeys, nextKeyDigests, signingKey) {
  const timestamp = (/* @__PURE__ */ new Date()).toISOString();
  const prior = toHex4(sha2567(new TextEncoder().encode(JSON.stringify({
