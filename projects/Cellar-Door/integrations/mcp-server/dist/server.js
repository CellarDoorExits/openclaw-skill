...
import { quickEntry, evaluateAdmission, verifyTransfer, OPEN_DOOR, STRICT, EMERGENCY_ONLY, } from "cellar-door-entry";
export function createServer(options = {}) {
            ...
            content: [
                {
                    ...
                    text: JSON.stringify({
                        did: identity.did,
                        message: "Identity generated and stored server-side for this session. Use create_exit_marker or quick_exit to sign markers. Private key material is not exposed.",
                    }, null, 2),
                },
            ],
        };
    ...
    //
    server.tool("verify_and_admit", "Verify an EXIT marker, evaluate an admission policy, and create a signed arrival marker", {
            ...
            if (!admission.admitted) {
                return {
                    content: [
                        {
                            type: "text",
                            text: JSON.stringify({ admitted: false, reasons: admission.reasons }, null, 2),
                        },
                    ],
                };
            }
    ...
    //
    server.tool("evaluate_admission", "Check whether an EXIT marker meets an admission policy without creating an arrival", {
        exitMarkerJson: z.string().describe("JSON string of the EXIT marker"),
        policy: z
            .enum(["OPEN_DOOR", "STRICT", "EMERGENCY_ONLY"])
    ...
    server.tool("verify_transfer", "Verify a complete EXIT→ENTRY transfer: check both markers and continuity", {
        exitMarkerJson: z.string().describe("JSON string of the EXIT marker"),
    ...
    server.tool("list_admission_policies", "List available admission policy presets and their configurations", {}, async () => {
        return {
            content: [
                {
                    type: "text",
