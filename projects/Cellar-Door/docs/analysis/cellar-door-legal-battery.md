...
> **📊 Risk Scale Reference:** This document uses text ratings (Low/Medium/High/Critical). See [projects/cellar-door-exit/docs/analysis/risk-scale-mapping.md](projects/cellar-door-exit/docs/analysis/risk-scale-mapping.md) for how this maps to scales used in other analysis documents.
...
This Battery provides seven independent specialist analyses: defamation, FCRA, Howey/securities, GDPR DPIA, international jurisdiction, cryptographic security, and insurance. Each analysis is self-contained with a risk rating and recommended action.

### What This Document Does NOT Cover

The following topics are addressed by other analysis documents in this suite:

**Covered by Red Team v2** (`docs/analysis/cellar-door-legal-redteam-v2.md`):
- **Antitrust / competition law** — RT2 §2.3 analyzes coordinated exit as potential refusal to deal under Sherman Act §1 and EU competition law. The Battery does not address antitrust risk.
- **Insurance reliance by protocol users** — RT2 §2.4 analyzes how insurers may rely on EXIT markers and the implications of self-attested status in insurance contexts. Battery §VII covers insurance for *Cellar Door itself*, not for downstream users.
- **Agent personhood and labor law framing** — RT2 §2.1–2.2 explores how evolving agent legal status affects the protocol. The Battery assumes current legal classifications.
- **Treaty and international humanitarian law** — RT2 §2.5 briefly touches on cross-border treaty obligations. Battery §V covers jurisdiction but not treaty-specific analysis.
- **Remediation quality grading** — RT2 Part III grades how well prior recommendations were implemented. The Battery provides fresh analysis, not remediation assessment.

**Covered by Legal Lenses** (`docs/analysis/cellar-door-legal-lenses.md`):
- **Multi-lens legal classification analysis** — Lenses examines EXIT under 11 different legal interpretations of what AI agents are (persons, property, employees, fiduciaries, etc.). The Battery assumes a single pragmatic framing per section rather than exhaustive multi-lens analysis.
- **Cross-lens conflicts and contradictions** — Lenses identifies where different legal frameworks produce contradictory requirements (e.g., privacy vs. owner visibility, right to exit vs. fiduciary duty). The Battery treats each legal domain independently.
- **Conservative baseline synthesis** — Lenses provides a "floor" of requirements that satisfy all lenses simultaneously. The Battery provides per-domain recommendations without cross-domain synthesis.
- **Recommended LEGAL.md spec language** — Lenses provides draft normative language for new LEGAL.md sections. The Battery recommends actions but doesn't draft language.
- **Open questions requiring case law** — Lenses identifies 8 unresolvable questions (functional erasure, agent signatures, etc.). The Battery flags risks but doesn't catalog unknowns.

**Covered by neither (gaps):**
- Detailed EU AI Act conformity assessment (flagged in Battery §V, Lenses Lens 9, but no dedicated analysis)
- Formal patent landscape search (flagged in Battery §VI as IP insurance consideration)
- State-by-state US analysis beyond Delaware and California
