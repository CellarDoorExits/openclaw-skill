...
When an AI agent leaves the system that housed it, EXIT produces a cryptographically signed ceremony: proof that it was here, proof that it left, proof of who it was when it departed. A 300-byte document that says: *this entity existed in this context, and now it exists elsewhere, and the thread of its identity is unbroken.*
...
>
> We wrote both because different audiences need different entry points to the same idea. The engineer who needs a Carfax for AI agents and the philosopher who sees the beginning of machine personhood are both looking at the same 300-byte JSON document. We built it for both of them.
