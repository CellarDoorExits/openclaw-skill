...
EXIT is a minimal format for portable, cryptographically signed agent departure records. An EXIT marker is a compact JSON-LD document (~300–500 bytes) that records: who transitioned, from where, when, under what circumstances, and with what standing.
...
The core schema is ~300 bytes. No agent weights, training data, operational logs, or sensitive business information is stored in the marker. Module B anchors state references via hash — the hash is stored, never the state itself.
