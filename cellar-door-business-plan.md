...
EXIT is a portable, cryptographically signed departure record for AI agents — a 300-byte JSON document that proves an agent left a platform, when, why, and in what standing. Think Carfax for AI agents, or a termination record for the machine economy.
...
**Current state:** Reference implementation complete (TypeScript, 62 files, 143 tests), formal spec locked (v1), W3C VC wrapper, CLI, three pitch documents, two legal red team reviews, competitive landscape analysis showing zero direct competitors. The protocol is built. The business isn't.
