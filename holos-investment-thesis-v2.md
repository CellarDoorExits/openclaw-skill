...
**What it is:** A portable, cryptographically signed departure record for AI agents — a 300-byte JSON proving an agent left a platform, when, why, and in what standing. The reference implementation is complete (TypeScript, 62 files, 143 tests). Open source, Apache 2.0.
...
### 1.7 Signamancy (Reputation) — Agent Reputation Systems
...
**Risk level:** Low financial risk (don't build until demand exists). High dependency risk — Signamancy is only as valuable as the data flowing through EXIT and NAME.
...
**Dependencies:** Heavy. Requires EXIT (data source), NAME (identity), Signamancy (reputation scoring). This is a top-layer application sitting on the full stack.
...
| **Signamancy** | $0 | 2% (defer) | Wait for data. |
...
- **HOLLOW + NAME + Signamancy:** Sovereign agents with identity and reputation = the trust infrastructure for agent commerce.
...
- Highest revenue ceiling in the portfolio. But depends on the full stack (EXIT + NAME + Signamancy) and 18+ months of data accumulation.
...
**#9: Signamancy** — Needs data that doesn't exist yet.
